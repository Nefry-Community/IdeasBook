...
これらのボードでは通常、「測定値を入力（インプット）」するボタン、温度計、加速度計といったセンサを使うときや、
「処理結果を出力（アウトプット）」するLED、サーボモータ、液晶ディスプレイといったアクチュエータの部品を使うとき、
ジャンパーワイヤやブレッドボードで「GPIO端子@<fn>{GPIO}」に接続したり、半田付けなどで接続したりします。

接続には回路の知識が必要だったり、はんだ付け技術が必要だったりと、電子工作初心者には少々敷居の高い作業になります。

//image[010_GROVE-Cable-Module-NefryBTR2-separated][GroveケーブルとGroveモジュール][scale=0.9]{
//}
Nefry BTでは、上記のような接続もできますが、Groveコネクタを搭載しているのでセンサやアクチュエータを直接挿すだけで手軽に試すことができます！
...
Groveモジュールの動作電圧は5Vが基本です。Nefry BTのGroveコネクタが供給する電圧は3.3Vなので、すべてのGroveモジュールが動作するわけではありませんが、多くのGroveモジュールは3.3Vに対応しています。
...
Groveシステムでは、IoTボードとGroveモジュール間を4線のGroveケーブルで接続します。4本の線の役割は次のとおりです。
...
//footnote[GPIO][General Purpose Inoput/Outputの略。外部機器とデータの入出力を行うための多用途の端子。ボード上にソケットやピンとして装備されている。]
