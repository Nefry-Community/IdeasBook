...
Nefry BT無印に接続する場合はA2のGroveソケット、Nefry BT R2に接続する場合はA0(プログラム上はA1)のGroveソケットに接続します。

下記のプログラムはNefry BT R2用なので、Nefry BT無印の場合はプログラム内の@<code>{#define PIN A1}を
@<code>{#define PIN A2}に書き換えてください。
...
Nefry BT無印ではGroveコネクタA2だけ、Nefry BT R2ではGroveコネクタA0だけ(プログラム上はA1)が使えるという結果になりましたが、
これには理由があります。
