= GroveのI2C（温度・湿度センサモジュール）
温度・湿度センサ（SHT31）はI2C接続のモジュールです。

//image[400-GROVE-MODULE-SHT31][Grove温度・湿度センサ(SHT31)][scale=0.4]{
//}

 * Grove温湿度センサ（SHT31） @<href>{https://www.switch-science.com/catalog/2853/}

I2C接続ではSCLとSDAという2本の信号線を使います。

Nefry BTでは、Grove D0コネクタにセンサを繋いで使用します。

== I2C温度・湿度センサ制御ライブラリの入手とセットアップ
...
== I2C温度・湿度センサモジュールのプログラム
1秒ごとに温度湿度データをシリアルモニタに送るプログラムです。
...
=== 実行結果
このプログラムを実行すると、シリアルモニタに測定結果が表示されます。

//image[480-MONITOR-SHT31][SHT31プログラムのシリアルモニタ表示例][scale=0.9]{
//}

温度・湿度モジュールと通信して、温度と湿度の情報がちゃんと表示されているのが確認できると思います！

====[column] Nefry Libraryのバージョンを上げよう
Nefry BT R2はライブラリのバージョンがV1.2.0以前だとI2C制御部分のプログラムがうまく動作しません。

そのため、ライブラリのバージョンをv1.2.1以降にしておきます。
...
//image[370-IDE-BoardManager-Nefry-v121][ボードマネージャの更新][scale=0.9]{
//}
