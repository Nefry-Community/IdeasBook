...
//image[top_2000seriese][2018年3月で引退した往年の名車2000形]{
//}
...
//image[dala_2go][以前作成したESP8266を使ったﾀﾞｧ2号]{
//}
...
//image[nefry_in_train][プチ電車シリーズにピッタリ収まるNefry BT]{
//}
...
//image[cooking_paper][クッキングペーパーを貼ると光が拡散する]{
//}
...
まず運行情報ですが、鉄道遅延情報のjson（https://rti-giken.jp/fhc/api/train_tetsudo/）から取得します。遅延している路線の一覧をJSON形式で取得できるので、Nefry BTでは@<tt>{HTTPClient}を使って、そのJSONを取得します。
...
先程のコードは京急線の遅延検知をしていますが、@<tt>{if (trainDelayJson.indexOf("京急") > 0)}の部分を書き換えれば、様々な路線に対応できます。また、今回はNefry BTに実装されているLEDだけを使っていますが、GPIOやGrove端子で接続された別のLEDを光らせることも可能です。
